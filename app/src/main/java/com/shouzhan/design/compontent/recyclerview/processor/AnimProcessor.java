...
public class AnimProcessor implements IAnimRefresh, IAnimOverScroll {
    ...
    private static final int MIN_SPEED = 5000;
    /**
     * 动画的变化率
     */
    ...
    public AnimProcessor(TwinklingRefreshLayout.CoContext coProcessor) {
        this.cp = coProcessor;
        mInterpolator = new DecelerateInterpolator(8);
    }
    ...
    public void scrollHeadByMove(float moveY) {
        float offsetY = mInterpolator.getInterpolation(moveY / cp.getMaxHeadHeight() / 2) * moveY / 2;
    ...
    public void scrollBottomByMove(float moveY) {
        float offsetY = mInterpolator.getInterpolation(moveY / cp.getMaxBottomHeight() / 2) * moveY / 2;
    ...
    public void animBottomBack(final boolean isFinishLoading) {
        ...
        animLayoutByTime(getVisibleFootHeight(), 0, new AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                //列表中加载到内容时滚动List
                if (!ScrollingUtils.isViewToBottom(cp.getTargetView(), cp.getTouchSlop())) {
                    ...
                    if (dy > 0) {
                        if (cp.getTargetView() instanceof RecyclerView) {
                            ScrollingUtils.scrollAViewBy(cp.getTargetView(), dy);
                        } else {
                            ScrollingUtils.scrollAViewBy(cp.getTargetView(), dy / 2);
                        }
                    }
                }
