...
import com.shouzhan.design.compontent.recyclerview.callback.OnGestureListener;
import com.shouzhan.design.compontent.recyclerview.callback.OnRefreshListener;
import com.shouzhan.design.compontent.recyclerview.callback.PullListener;
import com.shouzhan.design.compontent.recyclerview.processor.AnimProcessor;
import com.shouzhan.design.compontent.recyclerview.processor.IDecorator;
import com.shouzhan.design.compontent.recyclerview.processor.OverScrollDecorator;
import com.shouzhan.design.compontent.recyclerview.processor.RefreshProcessor;
...
/**
 * @author Created by lcodecore on 16/3/2.
 */
...
public class TwinklingRefreshLayout extends RelativeLayout implements PullListener, NestedScrollingChild {
     ...
     */
    private static String mHeaderClassName = "";
    private static String mFooterClassName = "";
    ...
    public TwinklingRefreshLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        try {
            mMaxHeadHeight = a.getDimensionPixelSize(R.styleable.TwinklingRefreshLayout_tr_max_head_height, dp2px(context, 120));
            mHeadHeight = a.getDimensionPixelSize(R.styleable.TwinklingRefreshLayout_tr_head_height, dp2px(context, 60));
            mMaxBottomHeight = a.getDimensionPixelSize(R.styleable.TwinklingRefreshLayout_tr_max_bottom_height, dp2px(context, 120));
            mBottomHeight = a.getDimensionPixelSize(R.styleable.TwinklingRefreshLayout_tr_bottom_height, dp2px(context, 60));
            mOverScrollHeight = a.getDimensionPixelSize(R.styleable.TwinklingRefreshLayout_tr_over_scroll_height, (int) mHeadHeight);
            enableRefresh = a.getBoolean(R.styleable.TwinklingRefreshLayout_tr_enable_refresh, true);
            enableLoadMore = a.getBoolean(R.styleable.TwinklingRefreshLayout_tr_enable_load_more, true);
            isPureScrollModeOn = a.getBoolean(R.styleable.TwinklingRefreshLayout_tr_pureScrollMode_on, false);
            isOverScrollTopShow = a.getBoolean(R.styleable.TwinklingRefreshLayout_tr_over_scroll_top_show, true);
            isOverScrollBottomShow = a.getBoolean(R.styleable.TwinklingRefreshLayout_tr_over_scroll_bottom_show, true);
            enableOverScroll = a.getBoolean(R.styleable.TwinklingRefreshLayout_tr_enable_over_scroll, true);
    ...
    private void addHeader() {
        ...
        if (mHeadView == null) {
            if (TextUtils.isEmpty(mHeaderClassName)) {
                return;
            }
            try {
                Class headClazz = Class.forName(mHeaderClassName);
    ...
    private void addFooter() {
        ...
        bottomViewLayout.setBackgroundColor(Color.WHITE);
        mBottomLayout = bottomViewLayout;
        this.addView(mBottomLayout);

        if (mBottomView == null) {
            if (TextUtils.isEmpty(mFooterClassName)) {
                return;
            }
            try {
                Class clazz = Class.forName(mFooterClassName);
    ...
    protected void onFinishInflate() {
        ...
        mCoContext.init();
        decorator = new OverScrollDecorator(mCoContext, new RefreshProcessor(mCoContext));
        initGestureDetector();
    }
    ...
    public void addFixedExHeader(final View view) {
        if (view != null && mExtraHeadLayout != null) {
            ...
            mCoContext.onAddExHead();
            mCoContext.setExHeadFixed();
        }
    }
    ...
    public void setMaxHeadHeight(float maxHeightDp) {
        setMaxHeaderHeight2(dp2px(getContext(), maxHeightDp));
    }

    public void setMaxHeaderHeight2(float maxHeight) {
        this.mHeadHeight = maxHeight;
    }
    ...
    public void setHeaderHeight(float headHeightDp) {
        this.mHeadHeight = dp2px(getContext(), headHeightDp);
    }

    public void setMaxBottomHeight(float maxBottomHeightDp) {
        setMaxBottomHeight2(dp2px(getContext(), maxBottomHeightDp));
    }

    public void setMaxBottomHeight2(float maxBottomHeight){
    ...
    public void setBottomHeight(float bottomHeightDp) {
        this.mBottomHeight = dp2px(getContext(), bottomHeightDp);
    }
    ...
    public void setOverScrollHeight(float overScrollHeightDp) {
        this.mOverScrollHeight = dp2px(getContext(), overScrollHeightDp);
    }
    ...
    public void onPullingDown(TwinklingRefreshLayout refreshLayout, float fraction) {
        ...
        if (refreshListener != null && enableLoadMore) {
    ...
    public void onFinishRefresh() {
        ...
        if (!mCoContext.isEnableKeepIView() && !mCoContext.isRefreshing()) {
            return;
        }
        mHeadView.onFinish(new OnAnimEndListener() {
            @Override
            public void onAnimEnd() {
                mCoContext.finishRefreshAfterAnim();
            }
        });
    }
    ...
    public void onFinishLoadMore() {
        ...
        if (!mCoContext.isEnableKeepIView() && !mCoContext.isLoadingMore()) {
            return;
        }
